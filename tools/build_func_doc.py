#!/usr/bin/env python
"""Render FUNCTIONAL_REQUIREMENTS.md into a professionally formatted Word document.
...
Mirrors tools/build_tech_doc.py.

Usage:
    /opt/anaconda3/bin/python tools/build_func_doc.py
"""
...
SRC = os.path.join(ROOT, "FUNCTIONAL_REQUIREMENTS.md")
OUT = os.path.join(ROOT, "Northeastern_Innovation_Dashboard_Functional_Requirements.docx")
...
def build_cover(doc):
    ...
    run = title.add_run("Functional Requirements\nDocument")
    run.bold = True
    run.font.size = Pt(30)
    run.font.color.rgb = NU_RED
    run.font.name = BODY_FONT
    ...
    meta = [
        ("Document type", "Functional requirements / product handoff reference"),
        ("Audience", "Incoming project manager, stakeholders, and product team"),
...
def add_footer(doc):
    ...
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run("Northeastern University Innovation Intelligence Dashboard — Functional Requirements")
    run.font.size = Pt(8)
    run.font.color.rgb = NU_GREY
    run.font.name = BODY_FONT
